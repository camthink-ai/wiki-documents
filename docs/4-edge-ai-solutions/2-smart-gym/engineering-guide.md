...
| 2 | PoE 交换机 | 千兆,端口数 = 相机数 + 1 | 1 台 | 相机供电与组网 |
| 3 | [**NG4500 AI Box**](https://www.camthink.ai/product/neoedge-ai-box-ng4500/)(或任意 Linux/macOS 主机) | 4GB+ 内存,Docker 环境 | 1 台 | 运行 NeoMind 平台 + gym-tracker 扩展 |
| 4 | 健身房安装包 | `gym-suite-<版本>.tar.gz`(向 CamThink 获取) | 1 套 | 一键安装(含应用镜像、模型、手册) |
| 5 | 网线 | Cat5e 以上 | 按需 | 相机 → PoE 交换机 → 主机 |

> 起步配置(单区域 ≤20 人):**1 台 NE503 + 1 台 PoE 交换机 + 1 台 NG4500**;场馆扩大时按区域增加相机,扩展配置里添加设备即可(单扩展建议 ≤4 路)。

## 3. 组网拓扑

```
┌────────┐  PoE 网线  ┌──────────┐   局域网   ┌──────────────────┐
│ NE503  │──────────▶│ PoE 交换机 │──────────▶│ NG4500(NeoMind)  │
│ 相机×N │           └──────────┘            │ gym-tracker 扩展  │
└────────┘                                   └────────┬─────────┘
                                                      │ OpenAPI / Data Push
                                              (可选)──▶ 业务系统 / 会员管理系统
...
- 相机与主机**同一局域网**,主机无需公网
- 带宽:每相机视频走 720p 子码流(约 2~4Mbps),事件流低于 100KB/s;4 路相机一台千兆交换机绰绰有余
...
### 4.1 安装 NeoMind 平台(NG4500 / 个人电脑)

方案主机推荐 NG4500 AI Box;评估阶段也可用 Mac mini 或任意 Linux 主机。
